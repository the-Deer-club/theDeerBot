  ...
  execute: async (client: CustomClient, interaction: CommandInteraction) => {
    try {
      ...
      const reason =
        ...
        'Bạn bị kick không vì gì cả! :3'

      const targetUser = await interaction.guild?.members.fetch(targetUserID)
      if (!targetUser) {
        await interaction.reply('Nun, người cần kick không tồn tại')
        return
      }

      if (targetUser?.id === interaction.guild?.ownerId) {
        await interaction.reply('Bạn không thể kick Sáng Trần được :3')
        return
      }
      ...
      await interaction.reply(
        `Nun, ${targetUser.displayName} đã bị đá khỏi clb :3`,
      )
    } catch (err) {
      console.log(err)
    }
